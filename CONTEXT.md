...
- **ADR-0011**: Agent harness selection is a formal runner seam (`images/agent-base/runners.py`): `AGENT_RUNNER` env (per-project via the registry's `agent_runner`, deployment-wide via Helm `temporalWorker.agentJob.runner`) selects `openhands` (default, behavior-preserving) or `claude-agent-sdk` (Anthropic models via their native harness). `runner.start(...)` returns a session; `session.send(message)` runs a turn and owns conversation continuity for the `QUESTION:` round-trip. Phase handlers, prompts, sentinels, and commit/push stay harness-portable in the entrypoint. Runner swaps should be measured (issue #122 eval flywheel) before defaults change.
